function [mllogbs, sdrel,loglikq,loglikmc,priorq,priormc,qq,qmc,mu,Qsim,Qsiminv,detQsiminv,qmall,qqall] = mlbsfullddmixpr5_sepsind_logit(alphamc,sepsmc,gammc,Zlogit,Qmc,Qinvmc,Qinvdet,prQnu,prQS,postQnu,postQS,prgamm,prgaminf,prs,postseps,prlambda,anmc,ancholmc,pralm,pralinf,y,ZG,dd)
...
gamsim=zeros(L,size(gammc,2),size(gammc,3));
qgamm=zeros(size(gammc,2),size(gammc,3));
qgamcov=zeros(size(gammc,2),size(gammc,2),size(gammc,3));

for k=2:size(gammc,3);
    eps = randn(size(gammc,2),L);
    qgamm(:,k)=mean(gammc(:,:,k),1)';qgamcov(:,:,k)=cov(squeeze(gammc(:,:,k)));
    gamsim(:,:,k) = (qgamm(:,k*ones(1,L)) + chol(qgamcov(:,:,k))'*eps)';
end
...
for k=2:size(gammc,3);
priormc = priormc + pmultnormlog(squeeze(gammc(:,:,k))',prgamm,prgaminf)';
priorq = priorq + pmultnormlog(squeeze(gamsim(:,:,k))',prgamm,prgaminf)';
end
...
for m=1:L
    ...
    loglikq(m) = simstateexstudent_logit(y,squeeze(gamsim(m,:,:)),Zlogit,ZG,Qqlik,betasim(:,m)',sepssim(1,m),prlambda,dd);
    loglikmc(m) = simstateexstudent_logit(y,squeeze(gammc(m,:,:)),Zlogit,ZG,Qmlik,alphamcmc(:,m)',sepsmcmc(m),prlambda,dd);
    
end   

[qq,qmc] = evalq5ddmix2_logit(qqall,qmall,betasim,sepssim,gamsim,alphamcmc,sepsmcmc,gammc,anmc,ancholmc,postseps,qgamm,qgamcov);
